...
```{r}
par(mfrow = c(1,2))
hist(mdims$hgt, main = "Men Height")
hist(fdims$hgt, main = "Female Height")
```

The histograms are approximately normal.Men and women height are centered around different mean.Females height is skewed to the left while males height is symmetrical.Mens height is uni-modal symmetric.
## The normal distribution

In your description of the distributions, did you use words like *bell-shaped* 
or *normal*?  It's tempting to say so when faced with a uni-modal symmetric 
distribution.
...
2.  Based on the this plot, does it appear that the data follow a nearly normal 
    distribution?
Answer: Yes. Based on the plot data appear to follow a nearly normal distribution.
...
qqline(sim_norm)
```
No all the plots don't fall on the line. This plot plots theoretical quarantines against sample quarantines.
...
Answer:The normal probability plot of fdmis$hgt look similar to the plots created for the simulated data. Plots provide evidence that female height are nearly normal.

5.  Using the same technique, determine whether or not female weights appear to 
    come from a normal distribution.
Answer:
```{r hist-height2 , eval=TRUE}
hist(fdims$wgt, probability = TRUE)
x <- 40:110
y <- dnorm(x = x, mean = mean(fdims$wgt), sd = sd(fdims$wgt))
lines(x = x, y = y, col = "blue")
```
```{r qqnormsim 2, eval=TRUE}
qqnormsim(fdims$wgt)
```
Female weight seems to be follow approximately bell-shaped curve, there are many values on  the normal QQ plots line. Female weight approximately follows normal distribution.
...
Q1. What is the probability that female height falls between 180 and 160 centimeter ?
```{r}
pnorm(180, mean = mean(fdims$hgt), sd = sd(fdims$hgt))-pnorm(160, mean = mean(fdims$hgt), sd = sd(fdims$hgt))
sum(fdims$hgt < 180& fdims$hgt > 160)/length(fdims$hgt)
```
The probabilities are theoretical 0.7613028 and empirical 0.7230769.
Q2. What is the probability that female weight more than 70 Kilogram?
```{r}
pnorm(70, mean = mean(fdims$wgt), sd = sd(fdims$wgt),lower.tail=FALSE)
sum(fdims$wgt > 70)/length(fdims$wgt)
```
The probabilities are theoretical 0.1641539 and empirical 0.1576923

weight had a closer agreement between he two methods.
...
Answer:
I think this might be due to the discrete nature of the variables.
...
```{r}
hist(fdims$kne.di, main = "Female Knee Diameter in cm")
```

From the histogram we can see that it aligns with the finding of normal quantile plot.
